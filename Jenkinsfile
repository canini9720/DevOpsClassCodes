pipeline{
	...
	stages	{
		stage('Checkout'){
			steps{
				git 'https://github.com/canini9720/DevOpsClassCodes'
			}
		}
		...
		stage('Deployment'){
			steps{
				echo "Deployment"
				sh 'sudo cp /var/lib/jenkins/workspace/AddressBookPipelineScript/target/addressbook.war /usr/share/tomcat/webapps/'
